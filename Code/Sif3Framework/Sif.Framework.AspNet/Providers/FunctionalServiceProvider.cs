...
namespace Sif.Framework.AspNet.Providers
{
    ...
    public class FunctionalServiceProvider : ApiController
    {
            ...
            try
            {
                ...
                return OkResult(
                    service.RetrieveToPhase(
                        ...
                        accept: HttpUtils.GetAccept(Request)));
            }
            catch (ArgumentException e)
            {
                throw new HttpResponseException(
            ...
            Right right)
        {
            ...
            try
            {
                RightsUtils.CheckRight(GetRights(serviceName, environment.GetTargetZone(zoneId?[0])), right);
        ...
        private string GetOwnerId(string sessionToken)
        {
            ...
            switch (ProviderSettings.EnvironmentType)
            {
                case EnvironmentType.DIRECT:
                    ...
                    ownerId = Request.Headers.GetApplicationKey() ?? sessionToken;
                    break;

                case EnvironmentType.BROKERED:
                    // Application key must have been moved into sourceName property
                    ownerId = Request.Headers.GetSourceName();
                    break;
            }
