...
namespace Sif.Framework.AspNet.RouteConstraints
{
    /// <summary>
    ...
    public class SegmentPrefixConstraint : IHttpRouteConstraint
    {
        ...
        /// <inheritdoc cref="IHttpRouteConstraint.Match(HttpRequestMessage, IHttpRoute, string, IDictionary{string, object}, HttpRouteDirection)" />
        public bool Match(
        ...
        {
            if (segmentPrefix == null) throw new ArgumentNullException(nameof(segmentPrefix));
            ...
            if (ControllerKey.Equals(segmentPrefix))
            {
                ...
                {
                    if (!Regex.IsMatch(segment, ControllerNamePattern)) continue;

                    string controllerName = Regex.Match(segment, ControllerNamePattern).Groups[1].Value;
                    match = GlobalConfiguration.Configuration.Services
                        .GetHttpControllerSelector()
                        .GetControllerMapping()
                        .ContainsKey(controllerName);
            ...
            {
                if (values.TryGetValue(segmentPrefix, out object value))
                {
