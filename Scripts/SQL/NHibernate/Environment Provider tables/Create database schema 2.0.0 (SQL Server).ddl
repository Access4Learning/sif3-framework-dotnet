...
alter table ENVIRONMENT  drop constraint FKC71F9BACD1CA0358
    ...
    if exists (select 1 from sys.objects where object_id = OBJECT_ID(N'[FKB343704D84903B70]') AND parent_object_id = OBJECT_ID('ENVIRONMENT_INFRASTRUCTURE_SERVICES'))
alter table ENVIRONMENT_INFRASTRUCTURE_SERVICES  drop constraint FKB343704D84903B70
...
alter table ENVIRONMENT_PROVISIONED_ZONES  drop constraint FKD922ED44E1C1FDE4
    ...
    if exists (select 1 from sys.objects where object_id = OBJECT_ID(N'[FK47B71BA2D1CA0358]') AND parent_object_id = OBJECT_ID('ENVIRONMENT_REGISTER'))
alter table ENVIRONMENT_REGISTER  drop constraint FK47B71BA2D1CA0358


    if exists (select 1 from sys.objects where object_id = OBJECT_ID(N'[FK86CD31CA84903B70]') AND parent_object_id = OBJECT_ID('ENVIRONMENT_REGISTER_INFRASTRUCTURE_SERVICES'))
alter table ENVIRONMENT_REGISTER_INFRASTRUCTURE_SERVICES  drop constraint FK86CD31CA84903B70
...
alter table ZONE_PROPERTIES  drop constraint FKDD117004D1CA0358
    ...
    if exists (select * from dbo.sysobjects where id = object_id(N'INFRASTRUCTURE_SERVICE') and OBJECTPROPERTY(id, N'IsUserTable') = 1) drop table INFRASTRUCTURE_SERVICE
    ...
    create table APPLICATION_INFO (
       ...
       DATA_MODEL_NAMESPACE NVARCHAR(256) null,
    ...
    create table ENVIRONMENT_INFRASTRUCTURE_SERVICES (
        ENVIRONMENT_ID UNIQUEIDENTIFIER not null,
       INFRASTRUCTURE_SERVICE_ID BIGINT not null,
       NAME INT not null,
    ...
    create table INFRASTRUCTURE_SERVICE (
        INFRASTRUCTURE_SERVICE_ID BIGINT IDENTITY NOT NULL,
       NAME INT null,
       [VALUE] NVARCHAR(256) null,
       primary key (INFRASTRUCTURE_SERVICE_ID)
    )
    ...
    create table [ZONE] (
        ZONE_ID BIGINT IDENTITY NOT NULL,
       SIF_ID NVARCHAR(64) null,
    ...
    alter table ENVIRONMENT_INFRASTRUCTURE_SERVICES 
        add constraint FKB343704D84903B70 
        foreign key (INFRASTRUCTURE_SERVICE_ID) 
        references INFRASTRUCTURE_SERVICE
    ...
    alter table ENVIRONMENT_REGISTER 
        add constraint FK47B71BA2D1CA0358 
        foreign key (ZONE_ID) 
        references [ZONE]

    alter table ENVIRONMENT_REGISTER_INFRASTRUCTURE_SERVICES 
        add constraint FK86CD31CA84903B70 
        foreign key (INFRASTRUCTURE_SERVICE_ID) 
        references INFRASTRUCTURE_SERVICE
